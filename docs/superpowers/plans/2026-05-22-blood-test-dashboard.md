...
```bash
cd dashboard
echo "test-key-123" | npx wrangler pages dev dist --binding DASHBOARD_KEY=test-key-123
```
...
Note: `Dashboard` does not exist until Task 15. `npm run typecheck` will fail until then — that is expected; do not commit a broken typecheck. This task's commit happens together with Task 15, or you may stub `Dashboard` temporarily. To keep commits clean, **defer this task's commit** and commit `App.tsx` in Task 15.

- [ ] **Step 2: Commit deferred**

`App.tsx` is committed in Task 15 once `Dashboard.tsx` exists. Proceed to Task 12.
...
Run: `cd dashboard && npm run typecheck`
Expected: fails only on the not-yet-existing `Dashboard` import in `App.tsx`; `FilterBar.tsx` itself reports no errors. (Confirm by reading the error output — it must name only `App.tsx`/`Dashboard`.)
...
Run: `cd dashboard && npm run typecheck`
Expected: fails only on the `Dashboard` import in `App.tsx`; the scorecard files themselves report no errors.
...
Run: `cd dashboard && npm run typecheck`
Expected: fails only on the `Dashboard` import in `App.tsx`; `TrendChart.tsx` reports no errors.
...
### Task 15: Dashboard screen wiring (`screens/Dashboard.tsx`) + commit `App.tsx`

**Files:**
- Create: `dashboard/src/screens/Dashboard.tsx`
- Commit: `dashboard/src/App.tsx` (written in Task 11)
...
- **Known deferral:** `App.tsx` is written in T11 but committed in T15 (after `Dashboard.tsx` exists) so no commit leaves the tree with a broken typecheck.
